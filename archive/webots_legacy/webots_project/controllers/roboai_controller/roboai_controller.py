...
from sensors import PoseSensorWrapper
...
def main():
    ...
    est = StateEstimator()
    pose_sensor = PoseSensorWrapper(robot, timestep=TIME_STEP_MS)
    ...
    seeded_pose = pose_sensor.read_pose()
    if seeded_pose is None:
        x0, y0, theta0 = resolve_initial_pose(world_name)
        seeded_pose = (float(x0), float(y0), float(theta0))
        seed_source = "world_default"
    else:
        seed_source = "gps_compass"
    est.reset_pose(x=float(seeded_pose[0]), y=float(seeded_pose[1]), theta=float(seeded_pose[2]))
    log.event(
        op="pose_seeded",
        x=float(seeded_pose[0]),
        y=float(seeded_pose[1]),
        theta=float(seeded_pose[2]),
        source=seed_source,
    )
    ...
    while elapsed < RUN_SECONDS:
        ...
        pose_source = "encoders"
        measured_pose = pose_sensor.read_pose()
        if measured_pose is not None:
            state.x = float(measured_pose[0])
            state.y = float(measured_pose[1])
            state.theta = float(measured_pose[2])
            pose_source = "gps_compass"
        ...
        log.event(
            ...
            pose_confidence=1.0 if pose_source == "gps_compass" else 0.65,
