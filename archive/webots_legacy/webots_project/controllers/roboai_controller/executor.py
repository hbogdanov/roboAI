...
from frontier import frontier_regions
...
class PlanExecutor:
    ...
    def _select_reachable_frontier(self, state, occ_grid, frontiers: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        for frontier in frontiers[:10]:
            tx, ty = frontier["centroid_world"]
            ...
            if not path_world:
                continue
            score = float(len(path_world)) - min(1.5, 0.08 * float(frontier.get("size", 0)))
            if score < best_score:
                best_score = score
                best_target = {
                    "target_xy": (tx, ty),
                    "size": int(frontier.get("size", 0)),
                    "distance_m": frontier.get("distance_m"),
                    "path_nodes": len(path_world),
                }
        return best_target
    ...
    def step(
        ...
        if op == "forward":
            ...
            if self.op_timer >= secs:
                ...
                if need_refresh:
                    frontier_candidates = frontier_regions(
                        occ_grid,
                        stride=2,
                        obstacle_clearance_cells=2,
                        robot_xy=(float(state.x), float(state.y)),
                        min_distance_m=0.18,
                        min_cluster_size=2,
                    )
                    self.log.event(op="frontier_detected", count=len(frontier_candidates))
                    target = self._select_reachable_frontier(
                        state=state, occ_grid=occ_grid, frontiers=frontier_candidates
                    )
                    if target is None and frontier_candidates:
                        self.log.event(op="frontier_unreachable", count=len(frontier_candidates))
                    self._frontier_target = None if target is None else target["target_xy"]
                    self._frontier_refresh_timer = 0.0
                    if target is not None:
                        self.log.event(
                            op="frontier_selected",
                            x=target["target_xy"][0],
                            y=target["target_xy"][1],
                            cluster_size=target.get("size", 0),
                            frontier_distance_m=target.get("distance_m"),
                            path_nodes=target.get("path_nodes", 0),
                        )
