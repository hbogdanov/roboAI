...
from frontier import frontier_points_world, frontier_regions  # noqa: E402
...
def test_frontier_regions_cluster_and_rank():
    prob = np.full((9, 9), 0.9, dtype=np.float32)
    prob[2:5, 2:5] = 0.2
    prob[1:6, 1:6][prob[1:6, 1:6] != 0.2] = 0.5
    g = DummyGrid(prob)
    regions = frontier_regions(g, stride=1, robot_xy=(0.0, 0.0), min_cluster_size=1)
    assert len(regions) > 0
    assert "centroid_world" in regions[0]
